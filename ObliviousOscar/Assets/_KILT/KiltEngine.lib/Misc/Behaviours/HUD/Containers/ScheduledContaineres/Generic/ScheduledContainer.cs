...
{	
	...
	[SerializeField]
	protected UnityEvent OnOpeningCallBack;
	[SerializeField]
	protected UnityEvent OnOpenedCallBack;
	[SerializeField]
	protected UnityEvent OnClosingCallBack;
	[SerializeField]
	protected UnityEvent OnClosedCallBack;
	...
	{
		if(PanelState != PanelStateEnum.Closed && CloseSpecialAction == CloseSpecialActionEnum.Deactivate)
			Hide(true);
	}
	...
	public void ForceSetPanelStateValue(PanelStateEnum p_value, bool p_updateEffect)
	{
		...
		else if (m_panelState == PanelStateEnum.Closed)
		{
			if(OnClosed != null)
				OnClosed();
		...
		else if(m_panelState == PanelStateEnum.Opening)
		{
			if(OnOpening != null)
				OnOpening();
		...
		else if(m_panelState == PanelStateEnum.Closing)
		{
			if(OnClosing != null)
				OnClosing();
	...
	public void Show(bool p_forceFinish)
	{
		...
		if(PanelState != PanelStateEnum.Opened)
		{
			...
			else if(RestartOption != RestartEnum.DontRestartIfRunning || 
	...
	public void Hide(bool p_forceFinish)
	{
		...
		if(PanelState != PanelStateEnum.Closed)
		{
			...
			else if(RestartOption != RestartEnum.DontRestartIfRunning || (PanelState != PanelStateEnum.Opening && PanelState != PanelStateEnum.Closing))
