...
public abstract class GlobalPressBehaviour : MonoBehaviour {
	...
	protected void HandleOnGlobalPress (bool p_pressed)
	{
		if(TriggerButton != MouseButtonTrigger.DontTrigger && 
		   ...
		   KiltUICamera.currentTouch != null && 
		   KiltUICamera.currentTouch.pressed != null &&
		   enabled && 
	...
	protected Vector2 GetWorldMousePosition(GameObject p_selectedObject)
	{
		...
		if(p_selectedObject != null)
		{
			...
			v_camera = CameraManager.GetCameraThatDrawLayer(p_selectedObject.layer);
			if(v_camera != null)
	...
	{
		ObjectPressed = KiltUICamera.currentTouch.pressed;
