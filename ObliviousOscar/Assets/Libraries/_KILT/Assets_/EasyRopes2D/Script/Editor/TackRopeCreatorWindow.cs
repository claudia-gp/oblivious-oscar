...
public class TackRopeCreatorWindow : RopeCreatorWindow
{
	...
	protected static AOTDictionaryKV<int,GameObject> m_selectedObjects = new AOTDictionaryKV<int,GameObject>();
	...
	protected void RecreateList(bool p_force = false)
	{
		if(_reordList == null || p_force || _needRecreateList)
		{
			...
			if(m_windowInformation != null && !m_windowInformation.PickSelectedObjects)
				_reordList = new UnityEditorInternal.ReorderableList(new List<AOTKeyValuePair<int,GameObject>>(m_selectedObjects.ToArray()), typeof(AOTKeyValuePair<int,GameObject>), true, true, true, true);
			else
				_reordList = new UnityEditorInternal.ReorderableList(new List<AOTKeyValuePair<int,GameObject>>(m_selectedObjects.ToArray()), typeof(AOTKeyValuePair<int,GameObject>), true, true, false, false);
